...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int k=1; k<= testCase; k++)
		{
			...
			minGroupResult = totalGroup;
			for(int i=1; i<= N; i++)
			{
				find(i);
				set.add(par[i].order); // 각 그룹의 대표값 저장 
			...
			for(int i=0; i< ans.size(); i++)
			{
				...
				dy1 = par[num].dy1;
				dy2 = par[num].dy2;
				int curOrder = par[num].order;
				Line target = new Line(dx1, -INF, dx1, INF);
				for(int j=1; j<= N; j++)
				{
					if(isCross(target, line[j]))
					{
						ap1 = tFind(j);
						if(ap1 != curOrder)
						{
				...
				if(minGroupResult > result) minGroupResult = result; 
				
				result = totalGroup;
				for(int j=1; j<= N; j++) tmp[j] = par[j].order;
				target = new Line(dx2, -INF, dx2, INF);
				for(int j=1; j<= N; j++)
				{
					if(isCross(target, line[j]))
					{
				...
				result = totalGroup;
				for(int j=1; j<= N; j++) tmp[j] = par[j].order;
				target = new Line(-INF, dy1, INF, dy1);
				for(int j=1; j<= N; j++)
				{
					if(isCross(target, line[j]))
					{
				...
				result = totalGroup;
				for(int j=1; j<= N; j++) tmp[j] = par[j].order;
				target = new Line(-INF, dy2, INF, dy2);
				for(int j=1; j<= N; j++)
				{
					if(isCross(target, line[j]))
					{
			...
			bw.write("#"+k+" "+totalGroup + " " + maxCrossCnt+" "+minGroupResult+"\n");
		}
		bw.flush();
	}
	...
	public static void tUnion(int a, int b)
	{
		int ap1 = tFind(a);
		int ap2 = tFind(b);
		tmp[ap1] = ap2;
	}
	public static int tFind(int a)
	{
		if(tmp[a] == a) return a;
		else return tmp[a] = tFind(tmp[a]);
	}
