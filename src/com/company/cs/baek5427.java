...
public class baek5427 {
	...
	public static void solve()
	{
		...
		while(!que.isEmpty())
		{
			...
			{
				Node n = que.poll();
				if(n.type == 0 && map[n.dx][n.dy] == '*') { // 같은 시간에서는 피할수 있지만 이전에 번진 불은 못피함 
					if(min > visit[1][n.dx][n.dy]) continue; // 불이 번진 시간 확인 
				}
				...
				for(int k=0; k<4; k++)
				{
					...
					if(map[nx][ny]== '*' || map[nx][ny]== '#' || visit[n.type][nx][ny] >= 0) continue;
